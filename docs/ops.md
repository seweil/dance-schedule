...
| --- | --- | --- | --- | --- |
| **CI** (`.github/workflows/ci.yml`) | Every push to `main`, and every PR | `checks` job (lint, typecheck, unit tests) + `e2e` job (full Playwright suite against a real build), then a `notify` job (push to `main` only) that emails a pass/fail summary — see `docs/testing.md` for the checks/e2e breakdown | GitHub → **Actions** tab → the `CI` run → its Summary page | Red ❌ on the run and on the commit/PR itself. On a push to `main`, also a **`SQDANCE: CI PASSED/FAILED — <short sha>`** email (via the same `dance-schedule-alerts` SNS topic the JS-error alarm uses), regardless of your own GitHub notification settings — explicit, not passive, and includes the commit hash. A PR run only gets the passive GitHub-notification signal (the `notify` job doesn't run for PRs — see its own comment in `ci.yml`). |

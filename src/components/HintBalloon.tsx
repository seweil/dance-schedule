...
export function HintBalloon({ message, onDismiss }: HintBalloonProps) {
  ...
  // PageMenu.tsx's real toggle button also counts as "outside" here and
  // dismisses it too, same as PageMenu.tsx's own handleToggleClick already
  // does explicitly — both paths call the same idempotent dismiss, so
  // there's no conflict from triggering it twice for that one tap. No

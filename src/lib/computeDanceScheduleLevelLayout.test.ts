...
describe('computeDanceScheduleLevelLayout', () => {
  ...
  it('places a single-level session in its slot column', () => {
    const session = makeSession('2026-07-02T13:00:00.000Z', '2026-07-02T14:00:00.000Z', 'Ballroom Centre', {
      levels: ['Plus'],
    })
  ...
  it('makes columnStart relative to the visible slot range, not the full slot list', () => {
    ...
    const layout = computeDanceScheduleLevelLayout([session], [session], SLOTS, 2, 5)
  ...
  it('shows the full filter-range column set even for a slot with nothing scheduled that day', () => {
    ...
    const layout = computeDanceScheduleLevelLayout([session], [session], SLOTS, 0, 3)
  ...
  it('gives a contiguous multi-level session a single spanning placement', () => {
    const session = makeSession('2026-07-02T13:00:00.000Z', '2026-07-02T14:00:00.000Z', 'Kafka/Lamartine', {
      levels: ['C1', 'C2'],
    })
    const layout = computeDanceScheduleLevelLayout([session], [session], SLOTS, 0, SLOTS.length - 1)
  ...
  it('collapses an A1/A2 session onto the merged slot when combineA1A2 is on', () => {
    const session = makeSession('2026-07-02T13:00:00.000Z', '2026-07-02T14:00:00.000Z', 'Salon A-C', {
      levels: ['A1', 'A2'],
    })
  ...
  it('floats a structured session tagged only Advanced/Intro/Various across every visible slot column', () => {
    ...
    const layout = computeDanceScheduleLevelLayout([session], [session], SLOTS, 0, 2)
  ...
  describe('overlap lanes', () => {
    it('assigns two sessions sharing a level at overlapping times to separate lanes', () => {
      const a = makeSession('2026-07-02T09:00:00.000Z', '2026-07-02T10:00:00.000Z', 'Ballroom Centre', {
        levels: ['C1'],
      })
      const b = makeSession('2026-07-02T09:30:00.000Z', '2026-07-02T10:30:00.000Z', 'Ballroom East', {
        levels: ['C1'],
      })
      const layout = computeDanceScheduleLevelLayout([a, b], [a, b], SLOTS, 0, SLOTS.length - 1)
    ...
    it('assigns three mutually overlapping sessions three lanes', () => {
      const a = makeSession('2026-07-02T09:00:00.000Z', '2026-07-02T10:00:00.000Z', 'Room A', { levels: ['C1'] })
      const b = makeSession('2026-07-02T09:15:00.000Z', '2026-07-02T10:15:00.000Z', 'Room B', { levels: ['C1'] })
      const c = makeSession('2026-07-02T09:30:00.000Z', '2026-07-02T10:30:00.000Z', 'Room C', { levels: ['C1'] })
      const layout = computeDanceScheduleLevelLayout([a, b, c], [a, b, c], SLOTS, 0, SLOTS.length - 1)
    ...
    it('does not narrow two sessions in the same column at non-overlapping times', () => {
      const morning = makeSession('2026-07-02T09:00:00.000Z', '2026-07-02T10:00:00.000Z', 'Room A', {
        levels: ['C1'],
      })
      const afternoon = makeSession('2026-07-02T13:00:00.000Z', '2026-07-02T14:00:00.000Z', 'Room B', {
        levels: ['C1'],
      })
      const layout = computeDanceScheduleLevelLayout(
        [morning, afternoon],
    ...
    it('a session ending exactly when another starts does not count as overlapping', () => {
      ...
      const layout = computeDanceScheduleLevelLayout([first, second], [first, second], SLOTS, 0, SLOTS.length - 1)
    ...
    it('decomposes a contiguous multi-level session into separate per-column placements when it also conflicts', () => {
      ...
      const combined = makeSession('2026-07-02T09:00:00.000Z', '2026-07-02T10:00:00.000Z', 'Kafka/Lamartine', {
        levels: ['C1', 'C2'],
      })
      const conflict = makeSession('2026-07-02T09:15:00.000Z', '2026-07-02T09:45:00.000Z', 'Salon 6/7', {
        levels: ['C1'],
      })
      const layout = computeDanceScheduleLevelLayout(
        [combined, conflict],
  ...
  it('sorts placements by rowStart then columnStart', () => {
    const later = makeSession('2026-07-02T14:00:00.000Z', '2026-07-02T15:00:00.000Z', 'Room B', { levels: ['C2'] })
    const earlier = makeSession('2026-07-02T13:00:00.000Z', '2026-07-02T14:00:00.000Z', 'Room A', {
      levels: ['C1'],
    })
    const layout = computeDanceScheduleLevelLayout([later, earlier], [later, earlier], SLOTS, 0, SLOTS.length - 1)

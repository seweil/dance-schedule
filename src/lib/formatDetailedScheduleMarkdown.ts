...
function formatLevels(session: DetailedSession): string {
...
function formatGca(session: DetailedSession): string {
...
/**
 * Produces a normalized, human-readable markdown dump of the parsed detailed
 ...
 * build time by vite-plugin-detailed-schedule.ts into data/detailed-schedule-dump.md.
 */
export function formatDetailedScheduleMarkdown(sessions: DetailedSession[]): string {
  const groups = groupDetailedSessionsByDate(sessions)

  return [
    '# Detailed schedule — parsed dump',
    '_Auto-generated by `vite-plugin-detailed-schedule.ts` at build time from' +
      ' `data/detailed-schedule.xlsx` — do not hand-edit._',

...
describe('shouldCombinePrimaryAndDetails', () => {
  ...
  it('accounts for the GCA line pushing a borderline case over the edge', () => {
    ...
    const inputs = { primaryText: 'SSD', detailsText: 'Ted Lizotte', textWidthPx: 1000, availableHeightPx: 50 }
    expect(shouldCombinePrimaryAndDetails({ ...inputs, hasGcaLine: false }, measureWidth)).toBe(false)
  ...
  it('never combines away a primary line that does not exist in the first place', () => {
    ...
    expect(
      shouldCombinePrimaryAndDetails(
        { primaryText: '', detailsText: 'Lunch Break', hasGcaLine: false, availableHeightPx: 80, textWidthPx: 1000 },
        measureWidth,
      ),
    ).toBe(false)
  })
